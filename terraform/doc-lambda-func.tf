...
resource "null_resource" "compile_binary" {
  ...
  provisioner "local-exec" {
    command = "GOOS=linux GOARCH=amd64 go build -ldflags '-w' -o  ../src/document_downloader/handler/main  ../src/document_downloader/handler/main.go"
  }
}

// zipping code
data "archive_file" "lambda_zip" {
  source_file = "../src/document_downloader/handler/main"
  type        = "zip"
...
resource "aws_lambda_function" "document_lambda" {
  function_name    = "assignment-docs-redirect"
